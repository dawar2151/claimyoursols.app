...
export const CloseMintAccountsManager = () => {
    ...
    const totalRent = mintAccounts.reduce((sum, account) => sum + account.lamports, 0);
    ...
    useEffect(() => {
        if (mintAccounts.length > 0) {
            const allAccountKeys = new Set(mintAccounts.map(account => account.pubkey.toString()));
            ...
            >
                ...
                {mintAccounts.length === 0 ? (
                    <div className="text-center py-12 flex flex-col justify-center items-center">
                        ...
                        <XTypography variant="h4" className="mb-2" style={{ color: colors.text.primary }}>
                            All Clean!
                        </XTypography>
                        <XTypography variant="body" className="text-center max-w-md" style={{ color: colors.text.secondary }}>
                            {"No open mintAccounts found. All your mintAccounts are already closed or you don't have any mintAccounts to close."}
                        </XTypography>
                    </div>
                ) : (
                    <div className="space-y-4">
                        {mintAccounts.map((account, _) => (
                            <div
                ...
                <div className="mt-8 pt-6 border-t" style={{ borderColor: `${colors.border}/50` }}>
                    ...
                    >
                        {isClosing ? "Closing Accounts..." : `Burn & Close Accounts & Get SOL Back (${selectedMintAccounts.size})`}
                    </XButton>
                    <XTypography variant="body" className="text-xs mt-3 text-center" style={{ color: colors.text.secondary }}>
                        This will close selected mintAccounts and refund your locked SOL back to your wallet
                    </XTypography>
                    {isSuccess && transactionHashes.length > 0 && (
                        ...
                        >
                            <div className="flex justify-between items-center mb-2">
                                <XTypography variant="body" className="text-sm font-semibold" style={{ color: colors.text.primary }}>
                                    Transaction Hashes
                                </XTypography>
                                <button
                            ...
                            <ul className="space-y-2">
                                {transactionHashes.map((hash, index) => (
                                    <li key={index}>
                                        <a
                                            href={getSolscanURL(claimYourSolsState.network, hash, "tx")}
                                            target="_blank"
